...
2. 仅将训练集按道路纵向空间区域映射至 5 个模拟 RSU；默认按目标样本比例确定连续空间边界，正式比例为 15%、17.5%、20%、22.5%、25%，使客户端保持可控的不均衡。每个客户端记录区域边界、车辆数、有效训练样本数与坐标统计。
3. 每辆车只能属于一个客户端，边界不得切分车辆；因此实际比例允许有小幅离散偏差。某区域样本不足时合并相邻区域，不得复制样本填充。车道或车辆分桶仅作次级诊断，不作为默认主划分。
